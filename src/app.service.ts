...
export class AppService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.ens = new ENS({ provider: this.provider, ensAddress: getEnsAddress('1') })
  ...
  async getAddressNouns(address: string, delegates: boolean = false): Promise<Noun[]> {
    return nounsForAddress(address, delegates);
  }

  async getAddressNounIds(address: string, delegates: boolean = false): Promise<number[]> {
    return R.map(R.prop('id'), await this.getAddressNouns(address, delegates)).sort((a, b) => a - b)
  }
  ...
  async getAddressNounTile(address: string, delegates: boolean = false): Promise<any> {
    const nounIds = await this.getAddressNounIds(address, delegates);
    const tileSideCount = Math.ceil(Math.sqrt(nounIds.length))
    const nounImageSideLength = Math.floor(constants.DEFAULT_HEIGHT / tileSideCount)
    const nounPngs = []
    ...
    for (let i in nounIds) {
      ...
      const png = (await this.getPng(nounId)).resize({width: nounImageSideLength})
